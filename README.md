...
### 6. Using ```object```s
The root is considered an implicit object. If we wrote out explict delimiters for the root it would be equivalent to ```[<PREFIX>d_root][<PREFIX>o]``` which would be the JSON object ```json {}```. By convention, we treat the named field for the root as if it was a variable ```typescript const root = {};```
